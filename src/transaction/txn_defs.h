    ...
    std::string GetInfo() {
        switch (abort_reason_) {
            ...
            case AbortReason::WRITE_CONFLICT: {
                return "Transaction " + std::to_string(txn_id_) + " aborted for write conflict\n";
            } break;

            case AbortReason::SERIALIZATION_CONFLICT: {
                return "Transaction " + std::to_string(txn_id_) + " aborted for serialization conflict\n";
            } break;
