            ...
            switch(x->tag) {
                case T_select:
                {
                    ...
                    return std::make_shared<PortalStmt>(PORTAL_ONE_SELECT, std::move(p->sel_cols_),
                                                        std::move(p->output_names_), std::move(root), plan);
                }
                ...
                case T_Delete:
                {
                    ...
                    std::unique_ptr<AbstractExecutor> root =
                        std::make_unique<DeleteExecutor>(sm_manager_, x->tab_name_, x->conds_, std::move(scan), context);
    ...
    std::unique_ptr<AbstractExecutor> convert_plan_executor(std::shared_ptr<Plan> plan, Context *context)
    {
        if(auto x = std::dynamic_pointer_cast<ProjectionPlan>(plan)){
            ...
            if (x->strategy_ == AggStrategy_Sort) {
                return std::make_unique<SortAggregationExecutor>(
                    std::move(input), x->agg_infos_, x->group_by_cols_,
                    x->having_conds_, x->sort_keys_);
            }
