#pragma once

#include <memory>
#include <utility>
#include <vector>

#include "executor_abstract.h"
#include "join_common.h"

class IndexNestedLoopJoinExecutor : public AbstractExecutor {
   private:
    std::unique_ptr<AbstractExecutor> left_;
    std::unique_ptr<AbstractExecutor> right_;
    JoinType join_type_ = INNER_JOIN;
    std::vector<Condition> conds_;
    ColMeta outer_lookup_col_;
    TabCol inner_lookup_col_;
    size_t left_len_ = 0;
    size_t right_len_ = 0;
    size_t eval_len_ = 0;
    size_t output_len_ = 0;
    std::vector<ColMeta> eval_cols_;
    std::vector<ColMeta> output_cols_;
    bool isend_ = true;
    Rid left_rid_{-1, -1};
    std::unique_ptr<RmRecord> left_rec_;
    std::unique_ptr<RmRecord> right_rec_;

    void set_end() {
        isend_ = true;
        left_rid_ = Rid{-1, -1};
        left_rec_.reset();
        right_rec_.reset();
    }

    void bind_right_lookup() {
        right_->bind_runtime_index_lookup(inner_lookup_col_, get_col_value(*left_rec_, outer_lookup_col_));
    }

    auto current_pair_matches() const -> bool {
        if (left_rec_ == nullptr || right_rec_ == nullptr) {
            return false;
        }
        return evaluate_join_pair_conditions(conds_, *left_rec_, *right_rec_, left_len_, right_len_, eval_cols_);
    }

    void seek_next_match(bool advance_left) {
        if (isend_) {
            return;
        }

        if (advance_left) {
            left_->nextTuple();
            if (left_->is_end()) {
                set_end();
                return;
            }
            left_rid_ = left_->rid();
            left_rec_ = left_->Next();
            bind_right_lookup();
            right_->restartTuple();
        }

        while (!left_->is_end()) {
            while (!right_->is_end()) {
                right_rec_ = right_->Next();
                if (current_pair_matches()) {
                    isend_ = false;
                    return;
                }
                right_->nextTuple();
            }
            left_->nextTuple();
            if (left_->is_end()) {
                break;
            }
            left_rid_ = left_->rid();
            left_rec_ = left_->Next();
            bind_right_lookup();
            right_->restartTuple();
        }

        set_end();
    }

   public:
    IndexNestedLoopJoinExecutor(std::unique_ptr<AbstractExecutor> left, std::unique_ptr<AbstractExecutor> right,
                                std::vector<Condition> conds, TabCol outer_lookup_col, TabCol inner_lookup_col,
                                JoinType join_type = INNER_JOIN)
        : left_(std::move(left)), right_(std::move(right)), join_type_(join_type), conds_(std::move(conds)) {
        if (join_type_ != INNER_JOIN && join_type_ != SEMI_JOIN) {
            throw InternalError("IndexNestedLoopJoinExecutor only supports inner and semi joins");
        }
        set_children({left_.get(), right_.get()});
        left_len_ = left_->tupleLen();
        right_len_ = right_->tupleLen();
        auto layout = build_join_schema_layout(left_->cols(), left_len_, right_->cols(), right_len_, join_type_);
        eval_len_ = layout.eval_len;
        eval_cols_ = std::move(layout.eval_cols);
        output_len_ = layout.output_len;
        output_cols_ = std::move(layout.output_cols);
        outer_lookup_col_ = *get_col(left_->cols(), outer_lookup_col);
        inner_lookup_col_ = std::move(inner_lookup_col);
        set_end();
    }

    void beginTupleImpl() override {
        left_->beginTuple();
        if (left_->is_end()) {
            set_end();
            return;
        }
        left_rid_ = left_->rid();
        left_rec_ = left_->Next();
        bind_right_lookup();
        right_->beginTuple();
        isend_ = false;
        seek_next_match(false);
    }

    void nextTuple() override {
        if (isend_) {
            return;
        }
        if (join_type_ == SEMI_JOIN) {
            seek_next_match(true);
            return;
        }
        right_->nextTuple();
        seek_next_match(false);
    }

    std::unique_ptr<RmRecord> NextImpl() override {
        if (isend_ || left_rec_ == nullptr || right_rec_ == nullptr) {
            return nullptr;
        }
        _abstract_rid = left_rid_;
        if (join_type_ == SEMI_JOIN) {
            return build_semi_output_record(*left_rec_, output_len_);
        }
        return build_join_eval_record(*left_rec_, *right_rec_, left_len_, right_len_);
    }

    Rid &rid() override { return _abstract_rid; }

    bool is_end() const override { return isend_; }

    size_t tupleLen() const override { return output_len_; }

    const std::vector<ColMeta> &cols() const override { return output_cols_; }
};
