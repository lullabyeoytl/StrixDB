...
class FileNotOpenError : public RMDBError {
   public:
    ...
    FileNotOpenError(const std::string &filename) : RMDBError("File not open: " + filename) {}
};
...
class DatabaseMetaCorruptedError : public RMDBError {
   public:
    DatabaseMetaCorruptedError(const std::string &db_name) : RMDBError("Database meta corrupted: " + db_name) {}
};
